...
import type { AppView, LearningHistoryEntry, StudentData, PersonalFolder } from '../types';
...
const HistoryEntryCard: React.FC<{
    ...
    folders: PersonalFolder[];
    onAssignHistoryToFolder: (historyId: string, folderId: string | null) => void;
}> = ({ entry, onContinue, onDelete, folders, onAssignHistoryToFolder }) => {
...
export const CourseFolderPage: React.FC<CourseFolderPageProps> = ({ history, studentData, setActiveView, onContinue, onDelete, folders, onAssignHistoryToFolder }) => {
                                            ...
                                            {courseHistory.length > 0 ? (
                                                <div className="space-y-4">
                                                    {courseHistory.map(entry => (
                                                        <HistoryEntryCard key={entry.id} entry={entry} onContinue={onContinue} onDelete={onDelete} folders={folders} onAssignHistoryToFolder={onAssignHistoryToFolder} />
                                                    ))}
                                                </div>
                                            ) : (
