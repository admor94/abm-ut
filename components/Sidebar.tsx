...
const colorClassMap: Partial<Record<AppView, string>> = {
    ...
    'Eksperimental': 'hover-bar-red',
};

const activeColorClassMap: Partial<Record<AppView, string>> = {
    'Dashboard': 'bg-ut-blue/20 text-ut-blue-light',
    'Belajar & Latihan': 'bg-ut-green/20 text-ut-green',
    'Produktivitas & Riset': 'bg-ut-yellow/20 text-ut-yellow',
    'Tutorial Online': 'bg-ut-red/20 text-ut-red',
    'Riwayat & Pelacakan': 'bg-ut-blue/20 text-ut-blue-light',
    'Seputar UT': 'bg-ut-blue/20 text-ut-blue-light',
    'Pengaturan & Bantuan': 'bg-slate-600/20 text-slate-200',
    'Eksperimental': 'bg-ut-red/20 text-ut-red',
};
...
const NavLink: React.FC<{
  ...
  const allowedWhenLocked: AppView[] = ['Dashboard', 'Pengaturan & Bantuan', 'Riwayat & Pelacakan', 'Seputar UT', 'FAQ', 'Dokumentasi Pembaruan', 'Tentang Aplikasi', 'Reset Aplikasi', 'Integrasi API', 'Pengaturan Lanjutan'];
  ...
  const isExperimental = view === 'Eksperimental';
  ...
  return (
    <a
      ...
      className={`${baseClasses} ${colorClass} ${isActive ? `active ${activeClasses} font-semibold` : inactiveClasses} ${isExperimental ? 'experimental-glow' : ''}`}
      aria-label={title}
...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isLocked, usedInviteCode }) => {
  const menuItems: { view: AppView, title: string, icon: React.ReactNode }[] = [
    ...
    { view: 'Pengaturan & Bantuan', title: 'Pengaturan & Bantuan', icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" /></svg> },
  ];
